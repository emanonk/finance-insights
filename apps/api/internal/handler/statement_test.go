...
func (f *fakeStatementService) Ingest(_ context.Context, fileName string, r io.Reader) (service.IngestResult, error) {
...
func TestStatementHandler_Create_HappyPath(t *testing.T) {
	...
	body, contentType := buildMultipart(t, "file", "statement.pdf", "application/pdf", []byte("%PDF-1.4\n%%EOF"))
...
func TestStatementHandler_Create_NonPDFRejected(t *testing.T) {
	...
	body, contentType := buildMultipart(t, "file", "notes.txt", "text/plain", []byte("hello"))
	req := httptest.NewRequest(http.MethodPost, "/statements", body)
	req.Header.Set("Content-Type", contentType)
	rec := httptest.NewRecorder()
...
func TestStatementHandler_Create_ServiceErrorReturns500(t *testing.T) {
	...
	body, contentType := buildMultipart(t, "file", "s.pdf", "application/pdf", []byte("%PDF"))
	req := httptest.NewRequest(http.MethodPost, "/statements", body)
	req.Header.Set("Content-Type", contentType)
	rec := httptest.NewRecorder()
